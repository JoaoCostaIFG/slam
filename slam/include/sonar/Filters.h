...
namespace sonar {
  ...
  /**
   *
   * Generates an image representation of a sweep under the cartesian system, for a given sweep.
   * Internal for displaySweep()
   * @param sweep the sweep from which to generate the image
   * @param img the image into which the image will be generated
   */
  static void generateSweepCartesian(const Sweep& sweep, cv::Mat& img) {
  ...
  void displaySweep(const Sweep& sweep, const bool inCartesian = false) {
    cv::Mat img;
    if (inCartesian) {
      generateSweepCartesian(sweep, img);
