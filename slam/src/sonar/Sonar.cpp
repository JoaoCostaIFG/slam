...
std::vector<Vector3<>>
Sonar::getBeamEndpoints2D(const Beam* beam, size_t obstacle_index) const {
...
std::vector<Vector3<>>
Sonar::getBeamEndpoints3D(const Beam* beam, size_t obstacle_index,
  ...
  double step_xoy = this->y_horiz / ndiv_horiz, low_horiz =
      beam->getAngle() - (this->y_horiz / 2);
  // Vertical Plane = yOz
